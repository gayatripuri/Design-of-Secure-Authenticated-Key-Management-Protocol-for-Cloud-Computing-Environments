...
<%
    try {
        ...
        ResultSet rs = sto.executeQuery("SELECT * FROM user_reg where details ='" + setvall + "' AND id ='" + id + "' ");
        if (rs.next()) {
            int i = sto.executeUpdate("update user_reg set logattempt='0' where id='" + id + "' ");
            response.sendRedirect("userHome.jsp?Success");
        } else {
            response.sendRedirect("userLogin.jsp?Msg=Incorrect_QR-code");
